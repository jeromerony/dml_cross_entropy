...
class BNInception(BaseModel):

    def __init__(self,
                 mean: Optional[Tuple[float, float, float]] = (104. / 255, 117. / 255, 128. / 255),
                 std: Optional[Tuple[float, float, float]] = (1 / 255, 1 / 255, 1 / 255),
                 **kwargs) -> None:
        self.backbone_features = 1024
    ...
    def feature_extractor(self, x: torch.Tensor) -> torch.Tensor:
        x = torch.flip(x, [1])  # bn_inception takes input in BGR
        ...
        return F.adaptive_avg_pool2d(inception_5b_output_out, output_size=1).flatten(1)


def bninception(num_classes: int = 100, pretrained: bool = True, **kwargs) -> nn.Module:
    r"""BNInception model architecture from <https://arxiv.org/pdf/1502.03167.pdf>`_ paper.
    """
    model = BNInception(num_classes=num_classes, **kwargs)
    if pretrained:
        model.load_state_dict(
            model_zoo.load_url('http://data.lip6.fr/cadene/pretrainedmodels/bn_inception-52deb4733.pth'),
            strict=False
        )
    return model
